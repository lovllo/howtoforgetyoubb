-- FULL: gass Exploit — tanpa Key System
...
local KeySettings = {
    ...
    Note = "Hubungi Fyy untuk mendapatkan key",
...
local Window = Rayfield:CreateWindow({
   Name = "Gass Exploit Menu",
   Icon = 0,
   LoadingTitle = "Gass Exploit Loader",
   LoadingSubtitle = "by Gass",
   ShowText = "Gass Exploit",
   Theme = "Ocean",
...
-- Daftar checkpoint
local XYZPositions = {
    Vector3.new(-430, 249, 790),
    Vector3.new(-347, 388, 522),
    Vector3.new(288, 429, 505),
    Vector3.new(334, 490, 347),
    Vector3.new(224, 314, -145),
...
arunikaTab:CreateButton({
    ...
    Callback = function()
        loadstring(game:HttpGet("https://raw.githubusercontent.com/fyybisnis4-gif/Fyy/refs/heads/main/auto%20arunika"))()
...
local BatuPositions = {
    ...
    Vector3.new(18, 573, 692),
    Vector3.new(595, 917, 621),
    Vector3.new(284, 1197, 182),
    Vector3.new(552, 1529, -581),
    Vector3.new(332, 1737, -261),
    Vector3.new(290, 1980, -204),
    Vector3.new(616, 3261, -66),
    Vector3.new(409, 3262, -112),
}

local MTBatuLoop = false

local function AutoClickSummitScreen()
    local plr = game:GetService("Players").LocalPlayer
    local gui = plr:WaitForChild("PlayerGui")
    local vu = game:GetService("VirtualUser")
    
    for _, obj in ipairs(gui:GetDescendants()) do
        if obj:IsA("TextButton") and obj.Text == "SUMMIT" and obj.AbsolutePosition then
...
CreateMenuToggle(mountTab, "MT Batu", false, function(Value)
    ...
    task.spawn(function()
        ...
        while MTBatuLoop do
            ...
            hrp = hrp or char:WaitForChild("HumanoidRootPart")
            task.wait(0.5)
            ...
            for i, pos in ipairs(BatuPositions) do
                ...
                -- Jika posisi terakhir, klik SUMMIT
                if i == #BatuPositions then
                    task.wait(1) -- tunggu button muncul
                    AutoClickSummitScreen()
                    task.wait(1)
                    -- Kembali ke basecamp
...
local YagatawPositions = {
    ...
    Vector3.new(-387, 1041, 2046),
    Vector3.new(-755, 1525, 2060),
    Vector3.new(-479, 1633, 2284),
    Vector3.new(-85, 1929, 2114),
